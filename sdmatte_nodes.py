...
def _ensure_sd21_configs():
    """
    检查 models/diffusers/stable-diffusion-2-1-base/ 是否存在。
    不存在则从插件自带的 assets/stable-diffusion-2-1-base.zip 自动解压（离线）。
    """
    try:
        diffusers_dir = os.path.join(folder_paths.models_dir, "diffusers")
        target_dir = os.path.join(diffusers_dir, "stable-diffusion-2-1-base")

        # 关键文件存在 → 已就绪
        if os.path.isfile(os.path.join(target_dir, "text_encoder", "config.json")):
            return True

        # 插件自带的 zip 路径
        plugin_dir = os.path.dirname(os.path.abspath(__file__))
        zip_path = os.path.join(plugin_dir, "assets", "stable-diffusion-2-1-base.zip")

        if not os.path.isfile(zip_path):
            print(f"[SDMatte] 未找到离线配置包: {zip_path}")
            return False

        print(f"[SDMatte] 缺少 SD 2.1 配置，正在从 {zip_path} 解压...")
        os.makedirs(diffusers_dir, exist_ok=True)

        with zipfile.ZipFile(zip_path, "r") as zf:
            zf.extractall(diffusers_dir)

        if os.path.isfile(os.path.join(target_dir, "text_encoder", "config.json")):
            print(f"[SDMatte] SD 2.1 配置解压完成: {target_dir}")
            return True
        else:
            print("[SDMatte] 解压完成，但未找到 text_encoder/config.json，请检查 zip 结构")
            print(f"[SDMatte] 期望路径: {target_dir}/text_encoder/config.json")
            return False

    except Exception as e:
        print(f"[SDMatte] 自动解压失败: {e}")
        return False
...
class SDMatteApply:
    ...
    def apply_matte(self, ckpt_name, image, trimap, inference_size, is_transparent, output_mode,
        ...
        # 确保 SD 2.1 配置就绪（离线自动解压）
        _ensure_sd21_configs()
